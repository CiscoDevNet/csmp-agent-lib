...
// NVM status codes
#define SL_STATUS_NVM_ERASE_PAGE_REQUEST        ((sl_status_t)0x0051)  ///< The Simulated EEPROM is telling application that at least one flashed page to be erased. The current page has also not been filled.
#define SL_STATUS_NVM_REPAIRING                 ((sl_status_t)0x0052)  ///< The Simulated EEPROM is repairing itself. While there's nothing for an app to do when the SimEE is going to repair itself (SimEE has to be fully functional for the rest of the system to work), alert the application to the fact that repair is
//// occurring. There are debugging scenarios where an app might want to know that repair is happening, such as monitoring frequency. Common situations will trigger an expected repair, such as using an erased chip or changing token definitions.
...
#define SL_STATUS_TRANSMIT_BLOCKED              ((sl_status_t)0x0058)  ///< The transmit attempt was blocked from going over the air. Typically
