...
#if SL_MEMORY_PROFILER_SELECTION
  char reservation_name[16] = "";
  sprintf(reservation_name, "Reservation %ld", sl_cmm_reservation_id_available);
#endif
#if SL_MEMORY_PROFILER_SELECTION == SL_MEMORY_PROFILER_SILABS
  sl_memory_profiler_track_alloc(sl_cmm_heap_tracker,
                                 handle->block_address,
                                 handle->block_size);
  sl_memory_tracker_handle_t tracker_handle = SL_INVALID_MEMORY_TRACKER_HANDLE;
  sl_memory_profiler_create_pool_tracker(&tracker_handle,
                                         sl_cmm_heap_tracker,
                                         reservation_name,
                                         handle->block_address,
                                         handle->block_size);
  sl_memory_profiler_save_runtime_tracker(tracker_handle, handle);
  sl_cmm_reservation_id_available++;
#endif
...
sl_status_t sl_memory_release_block(sl_memory_reservation_t *handle)
{
...
#ifdef SLI_MEMORY_MANAGER_ENABLE_TEST_UTILITIES
  // Fix for missing rounding up, if not we might lose a block forever.
  uint32_t alignment = sli_memory_get_reservation_align_by_addr(handle->block_address);
  if (alignment != SL_MEMORY_BLOCK_ALIGN_DEFAULT) {
    new_free_block_length = SLI_BLOCK_LEN_BYTE_TO_DWORD(SLI_ALIGN_ROUND_UP(handle->block_size, alignment));
  }
#endif
  ...
  if ((uintptr_t)new_free_block == (uintptr_t)handle->block_address) {
    // Update implicit double linked-list if a new free block is created.
    if (next_block != NULL) {
...
#ifdef SL_CATALOG_MEMORY_PROFILER_PRESENT
#if SL_MEMORY_PROFILER_SELECTION == SL_MEMORY_PROFILER_SILABS
  sl_memory_profiler_free_pool_memory(sl_cmm_heap_tracker, handle->block_address, handle->block_size);
#endif
#endif
...
/***************************************************************************//**
 * Get the size of the memory reservation handle structure.
 ******************************************************************************/
