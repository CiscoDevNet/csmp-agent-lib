...
#include "sl_memory_profiler.h"
#if SL_MEMORY_PROFILER_SELECTION == SL_MEMORY_PROFILER_SYSTEMVIEW
...
#ifdef SL_CATALOG_MEMORY_MANAGER_PROFILER_PRESENT
#if SL_MEMORY_PROFILER_SELECTION == SL_MEMORY_PROFILER_SYSTEMVIEW
...
/***************************************************************************//**
 * Allocate a memory block of at least requested size from the heap. Simple
 * version.
 ******************************************************************************/
  ...
  if (block_size_remaining >= SLI_BLOCK_ALLOCATION_MIN_SIZE) {
    ...
    if (type == BLOCK_TYPE_LONG_TERM) {
      ...
      if (current_block_metadata->offset_neighbour_next != 0) {
        ...
        allocated_blk->offset_neighbour_next = (uint16_t)SLI_BLOCK_LEN_BYTE_TO_DWORD(size_adjusted + SLI_BLOCK_METADATA_SIZE_BYTE);
...
#ifdef SL_CATALOG_MEMORY_PROFILER_PRESENT
#if SL_MEMORY_PROFILER_SELECTION == SL_MEMORY_PROFILER_SILABS
  // Preserve this order else the host-side will not record the events correctly.
  SL_MEMORY_PROFILER_TRACK_ALLOC_WITH_OWNERSHIP(sl_cmm_heap_tracker, allocated_blk, size_real + SLI_BLOCK_METADATA_SIZE_BYTE, sl_memory_profiler_get_return_address());
  SL_MEMORY_PROFILER_TRACK_ALLOC_WITH_OWNERSHIP(sl_cmm_heap_malloc_tracker, *block, size, sl_memory_profiler_get_return_address());

  if (type == BLOCK_TYPE_LONG_TERM) {
    SL_MEMORY_PROFILER_TRACK_ALLOC_WITH_OWNERSHIP(sl_cmm_heap_malloc_lt_tracker, *block, size, sl_memory_profiler_get_return_address());
  } else if (type == BLOCK_TYPE_SHORT_TERM) {
    SL_MEMORY_PROFILER_TRACK_ALLOC_WITH_OWNERSHIP(sl_cmm_heap_malloc_st_tracker, *block, size, sl_memory_profiler_get_return_address());
  }
#endif
...
#ifdef SL_CATALOG_MEMORY_PROFILER_PRESENT
// We place tracking at the top to track failure too.
#if SL_MEMORY_PROFILER_SELECTION == SL_MEMORY_PROFILER_SILABS
  SL_MEMORY_PROFILER_TRACK_FREE(sl_cmm_heap_malloc_tracker, block);
#endif
#endif
...
#ifdef SL_CATALOG_MEMORY_MANAGER_PROFILER_PRESENT
#if SL_MEMORY_PROFILER_SELECTION == SL_MEMORY_PROFILER_SILABS
  SL_MEMORY_PROFILER_TRACK_FREE(sl_cmm_heap_tracker, ((uint8_t *)block - SLI_BLOCK_METADATA_SIZE_BYTE));
#endif
#endif
  ...
  *block = NULL; // No block allocated yet.
  ...
  if (size_real > current_block_len) {
    ...
    *block = ptr;
    ...
    // And return the same given address.
    *block = ptr;
  }
...
/***************************************************************************//**
 * Manage the required data alignment by moving the non-aligned block payload
