 ...
 *****************************************************************************/
/**
 * @addtogroup Front End Module Voltage Detection (VDET)
 * @brief APIs for configuring and detecting a voltage from a Front End Module.
 *
 * The rail_util_vdet plugin lets customers measure an output voltage from their attached
 * Front End Module. This information allows optimizations to power configuration,
 * and monitoring FEM performance.
 *
 ...
 *   Begins IMMEDIATE measurement if in RAIL_VDET_MODE_IMMEDIATE.
 * @return Status code indicating success of the function call.
 */
RAIL_Status_t RAIL_EnableVdet(RAIL_Handle_t railHandle,
                              bool enable);
 ...
 * @return Status code indicating success of the function call:\n
 *   NO_ERROR: All went well, value in *vdet_mv is valid.\n
 *   INVALID_CALL: VDET is in progress, wait until VDET capture is complete and try again.\n
 *   INVALID_STATE: VDET is not enabled or valid, or VDET was cancelled due to AuxADC contention.\n
                    AUTOMATIC - Wait until next packet and try reading again.\n
                    IMMEDIATE - Resend measurement request.\n
 */
RAIL_Status_t RAIL_GetVdet(RAIL_Handle_t railHandle,
                           uint32_t *pVdetMv);
